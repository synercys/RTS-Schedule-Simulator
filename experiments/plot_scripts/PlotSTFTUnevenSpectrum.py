...
if __name__ == '__main__':
    ...
    # args = vars(parser.parse_args("--in /Users/jjs/Documents/myProject/RTS-Schedule-Simulator/experiments/test_cases/dft/1-task-33Hz-25U/rm-v-10-uneven.rtstft -r -p".split()))
    ...
    heatmapWidth = 1/lenTimeBins
    for i in range(lenTimeBins):
        # bounds = [x0, y0, width, height]
        print("Plotting the #{} bin ...".format(i+1))
...
#    ax.set_ylim(-1, idx_of_freq_upper_limit+1)    # start with -1 since index=0 (freq=0) was removed beforehand
    ...
    ax2.set_ylim(0, freq_upper_limit)
    ...
    # ax3.set_xlim(0, float(jsonRoot['data']['unevenSpectrum'][-1]['time'])*tickToMsMultiplier)
    ...
    ax3.set_xlim(0.5, lenTimeBins+0.5)
    ...
    if displayFreqRanking:
        ...
        # find the closest larger
        i = 0
        maxTaskFreqRanking = max(taskFreqRanking)
        while True:
            if 10**i > maxTaskFreqRanking:
                y_upper_lim = 10**i
                break
            i += 1

        if maxTaskFreqRanking == 1:
            axr.set_ylim(0.8, y_upper_lim)  # to let 1 visible
        else:
            axr.set_ylim(1, y_upper_lim)
        ...
        axr.get_yaxis().set_major_locator(ticker.LogLocator(numticks=y_upper_lim))
        locmin = ticker.LogLocator(base=10.0, subs=(0.2, 0.4, 0.6, 0.8), numticks=y_upper_lim)
