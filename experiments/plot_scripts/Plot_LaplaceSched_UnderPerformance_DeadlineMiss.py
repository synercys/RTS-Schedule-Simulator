...
def plot_mean_frequency_error_ratio(show_plot, in_csv_file_list, out_plot_file_list, label_list):
    ...
    for in_csv_idx in range(len(in_csv_file_list)):
        ...
        this_y = data_dict['Mean_Task_Under_Performance_Ratio']
    ...
    ax_top.tick_params(top=False, bottom=False, left=False, right=True, labelleft=False, labelbottom=False,
    ...
    ax_top.plot(range(10), tasksets_with_deadline_misses_count, marker='.',
                markersize=10, color='black')
    ax_top.text(0, 1.1, "The Number of Task Sets\nthat Have Deadline Misses\nin {}".format(label_list[-1]),
                transform=ax_top.transAxes, fontsize=14,
                verticalalignment='top', horizontalalignment='left')

    for i in range(10):
        if tasksets_with_deadline_misses_count[i] > 0:
            ax_top.text(i, tasksets_with_deadline_misses_count[i] + 10,
                        '$\\frac{' + str(tasksets_with_deadline_misses_count[i]) + '}{600}$', fontsize=12, ha='center',
                        va='bottom')

    ### post plot configurations ###
    plt.xlabel('Task Set Utilization')
    plt.ylabel('Mean Under Performance Ratio')

    plt.legend(loc='upper center', shadow=True, edgecolor='k', ncol=2)
...
def get_utilization_mean_precision_list(in_csv_file):
    ...
    precision_list = data_dict['Mean_Task_Under_Performance_Ratio']
