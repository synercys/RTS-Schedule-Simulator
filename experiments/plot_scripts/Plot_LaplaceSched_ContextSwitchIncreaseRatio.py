...
def plot_peak_count(show_plot, in_csv_file_list, out_plot_file_list, label_list):
    ...
    for i in range(len(y_list)):
        ...
        plt.scatter(x, y, marker=PlotUtility.pattern_list[style_idx], facecolors=PlotUtility.palletForMany[style_idx], color=PlotUtility.palletForMany[style_idx], alpha=0.5, s=[markerSize for i in range(len(y))], label=plot_labels[i])
        plt.errorbar(x_mean,y_mean,y_error,color="black",marker=PlotUtility.pattern_list[style_idx], linewidth=1)
