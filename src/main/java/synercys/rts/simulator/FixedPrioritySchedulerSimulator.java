...
/**
 * FixedPrioritySchedulerSimulator.java
 * Purpose: A simulator for the preemptive, fixed-priority real-time scheduler using the rate-monotonic (RM) priority assignment.
 *
 * @author CY Chen (cchen140@illinois.edu)
 * @version 1.0 - 2018, 12/14
 */
public class FixedPrioritySchedulerSimulator extends SchedulerSimulator implements Advanceable {

    // This map stores each task's next job instance, no matter it's arrived or not.
    HashMap<Task, Job> nextJobOfATask = new HashMap<>();

    public FixedPrioritySchedulerSimulator(TaskSet taskSet, boolean runTimeVariation) {
